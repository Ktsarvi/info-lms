...
begin
  ...
  if extract(day from target_expiry) < original_day then
    -- PostgreSQL already handles this correctly by rolling to month end
  ...
  set
    ...
    auto_renew = case when p_card_uuid is not null then true else auto_renew end
  where id = p_user_id;
